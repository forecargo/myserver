...
4ボタン（2×2）が設定される。白背景＋円形アイコン＋日本語ラベル＋英語サブラベルのモダンデザイン：

| ボタン | アイコン | 英語ラベル | 送信テキスト |
|---|---|---|---|
| 発生中 | 警告△（赤・ピンク背景） | ACTIVE（赤） | `発生中` |
| 一覧   | テーブル（グレー背景）   | HISTORY      | `一覧` |
| 同期   | 循環矢印（グレー背景）   | SYNC         | `同期` |
| ヘルプ | ?マーク（グレー背景）    | SUPPORT      | `ヘルプ` |

画像は `setup_richmenu.py` 内の `make_image()` で Pillow を使って動的生成。デザイン変更時は `make_image()` と各 `_draw_*` ヘルパー関数を修正して再実行。
...
「詳細を見る #ID」ボタンは `BASE_URL/?id=ID` を開き、Web UI でそのインシデントの詳細モーダルが自動表示される。
