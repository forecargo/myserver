...
- status: メール内容から以下のいずれかを判定してください:
  ...
  - "調査中": 原因調査中または対応中。根本原因調査中・恒久対応未完了の場合も「調査中」
  - "復旧済み": サービスが完全に復旧したことを正式に確認する最終報のみ使用すること。
    ※ 第一報・続報では原則「発生中」または「調査中」を使用すること。
    ※ 「復旧作業中」「復旧予定」「一部復旧」「根本原因調査中」は「調査中」とする。
...
2. report_type が「最終報」かつ本文に明確な復旧確認がある場合のみ status を「復旧済み」にする
3. report_type が「第一報」「続報」の場合、status は「発生中」または「調査中」のみ使用する
